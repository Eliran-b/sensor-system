...
from services.alert_service.main import alert_service_main
...
class TestAlertServiceFlow(BaseSensorSystemTest):
    ...
    def setUp(self) -> None:
        super().setUp()
        self.input_queues_list = [self.alert_service_queue]

    async def test_alert_service_flow_success(self):
        test_message_data_list = [
            {"message": "PRESSURE: 0 is not in valid range [900, 1100]", "sensor_name": "PRESSURE"},
